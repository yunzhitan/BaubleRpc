...
public class RpcConfigs {
    ...
    private static void init() {
        try {
            ...
            // loadCustom
            loadCustom("bauble-rpc/rpc-config.json");
            loadCustom("META-INF/bauble-rpc/rpc-config.json");

            // load system properties
            CFG.putAll(new HashMap(System.getProperties())); // 注意部分属性可能被覆盖为字符串
        } catch (Exception e) {
            throw new RuntimeException("Catch Exception when load RpcConfigs", e);
        }
    }
    ...
    public static boolean getBooleanValue(String primaryKey) {
        ...
        if (val == null) {
            throw new RuntimeException("Not found key: " + primaryKey);
        } else {
    ...
    public static boolean getBooleanValue(String primaryKey, String secondaryKey) {
        ...
        if (val == null) {
            ...
            if (val == null) {
                throw new RuntimeException("Not found key: " + primaryKey + "/" + secondaryKey);
            }
        }
        return Boolean.valueOf(val.toString());
    }
